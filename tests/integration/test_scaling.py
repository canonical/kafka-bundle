...
APPLICATIONS = ["zookeeper", "kafka"]
BUNDLE="releases/latest/k8s/kafka-k8s.yaml"
KUBERNETES=True if yaml.safe_load(Path(BUNDLE).read_text())["bundle"] == "kubernetes" else False 
...
async def test_deploy_active(ops_test: OpsTest):
    ...
    time.sleep(60)
    await ops_test.model.block_until(lambda: (len(ops_test.model.applications["zookeeper"].units) == 3 and len(ops_test.model.applications["kafka"].units) == 3))
...
@pytest.mark.abort_on_fail
async def test_zookeeper_simple_scale_up(ops_test: OpsTest):
    await ops_test.model.applications["zookeeper"].add_units(count=3)
    await ops_test.model.block_until(lambda: len(ops_test.model.applications["zookeeper"].units) == 6)
    await ops_test.model.wait_for_idle(apps=["zookeeper"], status="active", timeout=1000)
    assert ping_servers(ops_test)


@pytest.mark.abort_on_fail
async def test_zookeeper_simple_scale_down(ops_test: OpsTest):
    if not KUBERNETES:
    ...
    await ops_test.model.block_until(lambda: len(ops_test.model.applications["zookeeper"].units) == 3)
    await ops_test.model.wait_for_idle(apps=["zookeeper"], status="active", timeout=1000)
    assert ping_servers(ops_test)


@pytest.mark.abort_on_fail
async def test_zookeeper_scale_up_replication(ops_test: OpsTest):
    await ops_test.model.wait_for_idle(apps=["zookeeper"], status="active", timeout=1000)
    assert ping_servers(ops_test)
    ...
    await ops_test.model.block_until(lambda: len(ops_test.model.applications["zookeeper"].units) == 4)
    await ops_test.model.wait_for_idle(apps=["zookeeper"], status="active", timeout=1000)
    check_key(host=host, password=password)


@pytest.mark.abort_on_fail
async def test_zookeeper_kill_quorum_leader_remove(ops_test: OpsTest):
...
@pytest.mark.abort_on_fail
async def test_zookeeper_kill_juju_leader_remove(ops_test: OpsTest):
...
@pytest.mark.abort_on_fail
async def test_zookeeper_kill_juju_leader_restart(ops_test: OpsTest):
...
@pytest.mark.abort_on_fail
async def test_zookeeper_same_model_application_deploys(ops_test: OpsTest):
...
async def test_kafka_simple_scale_down(ops_test: OpsTest):
    if not KUBERNETES:
        ...
        await ops_test.model.block_until(lambda: len(ops_test.model.applications["kafka"].units) == 2)
        await ops_test.model.wait_for_idle(apps=["kafka"], status="active", timeout=1000)
